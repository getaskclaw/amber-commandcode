...
6. **Winds shifting on g47's 7 contested cases**: the same-day Nous control lane reproduced the same failure shape on 6 of those cases in a wall-free environment (plan-and-stop delivery failure; details in [amber-nous W39](https://github.com/getaskclaw/amber-nous)). Wall or disease — the 09-29 same-lane re-exam decides. The board keeps them contested; no early exoneration.

## Harness notes

- Multi-variant cases (A-0676097b, 4 runs sharing one bundle) count as one case and must go all-green: m26p went 4/4.
- g47 quota-wall evidence chain: billing API $14.02/$14 + 503 probes from two machines agreeing; the driver kill-switch stopped the exam, zero poisoned runs; the 7 contested cases are booked (`scoring-contested-cc-g47.txt`, commit 7743a12e); m26f's 2 invalid runs from a hot-window start were wiped and re-sat.
...
- **grok-4.7 (this lane)**: no rating while incomplete; on the runs it did sit, the attribution pins (14/15) are the highlight, build face 4/6. For delivery-reliability doubts see the amber-nous control board.
